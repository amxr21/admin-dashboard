...
| URG-025–032 | `[*]` | **URG-025 done and verified in a browser, both locales.** Default body is exactly the six basics (Image, Name*, Price*, Stock, Category, Status); seven groups render with correctly localized headings in en AND ar (no untranslated keys), all open, with SKU/weight/slug still reachable and zero console errors. **The browser caught something the tests could not**: `description` was silently sitting in the default body because I had never tagged it — a `longtext` is written deliberately, not rattled off with name/price/stock, so it now groups with the other content. A new optional `group` property on the shared `FieldConfig` drives progressive disclosure. Ungrouped fields render in the default body; grouped ones collect into named `CollapsibleSection`s reusing the URG-033 primitive rather than a second disclosure idiom. Per the owner's decision the default body keeps name, price, stock, category, image and status; cost, SKU, barcode, dimensions, shipping, SEO, tags and storage move into seven named groups. The schema endpoint forwards `config.fields` verbatim, so no route change was needed. Groups default to OPEN — load-bearing, not cosmetic: the section hides its region with the `hidden` attribute, so a closed group would drop every grouped input out of the accessibility tree and break the suite's `getByLabelText` queries. Typecheck/lint clean; 50/50 resource-form, 24/24 product panels. URG-026–032 not started. |
