 ...
 * the setting, and it would drift toward being WRONG — enabling the button for
 * a password the server then rejects. Instead the server's own 400 is surfaced
 * verbatim.
 *
 * Note the contrast with `staff-password-panel.tsx` and `staff-sheet.tsx`,
 * which DO show a live floor: both are authenticated surfaces, so they can
 * read `minPasswordLength` from `useAppSettings()`. This page cannot, and that
 * is the whole reason it behaves differently.
 *
