...
async function submit({
  ...
  await user.type(screen.getByLabelText(/reset code/i), token);
...
describe('client-side guards', () => {
  it('refuses mismatched passwords without calling the API', async () => {
    ...
    // Matches "don't match" and "do not match" — the assertion is that the
    // mismatch is REPORTED, not that the copy is phrased one particular way.
    expect(await screen.findByRole('alert')).toHaveTextContent(/do(n't| not) match/i);
  });
...
describe('error mapping', () => {
  ...
  it('keeps the token but clears the passwords after a failure', async () => {
    ...
    expect(screen.getByLabelText(/reset code/i)).toHaveValue('KEEPME');
