...
export function ReturnResolutionBreakdownView() {
  ...
  const query = useCallback(() => fetchReturnResolutionBreakdown(range), [range]);
  const { data, isLoading, error, setError, load } = useReportQuery<ReturnResolutionBreakdown>(query);
  ...
  return (
    <div className="space-y-4">
      ...
      ) : (data?.byResolution.length ?? 0) === 0 && (data?.byStatus.length ?? 0) === 0 ? (
        /**
