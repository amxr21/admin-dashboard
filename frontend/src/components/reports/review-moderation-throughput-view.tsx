...
export function ReviewModerationThroughputView() {
  ...
  const query = useCallback(() => fetchReviewModerationThroughput(range), [range]);
  const { data, isLoading, error, setError, load } = useReportQuery<ReviewModerationThroughput>(query);
